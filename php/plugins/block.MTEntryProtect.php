...
//          return $content;
			...
			elseif($type == "Typekey") {
				...
				if(empty($signintext))
					$signintext = "This entry has been Typekey protected so only selected Typekey users can read it. ";
				$notallowed = $args['tk_barred_text'];
				if(empty($notallowed)) 
					$notallowed = "You do not have the rights to access this entry. Sorry!";								
          ...
          if ($logged_in) {
			...
			elseif($type == "OpenID") {
				...
				if(empty($signintext))
					$signintext = "this entry has been protected using OpenID so only selected OpenID users can read it. ";
